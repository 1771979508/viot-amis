            ...
            link.icon &&
                (Array.isArray(link.icon) ? link.icon : [link.icon]).forEach(function (item, i) {
                    if (React__default["default"].isValidElement(item)) {
                        ...
                        var icon = (_J$X_(amisUi.Icon, { key: "icon-".concat(i), cx: cx, icon: item['icon'] || item, className: isCollapsedNode ? '' : isAfter ? 'ml-2' : 'mr-2' }));
                        if (isAfter) {
                            afterIcon.push(icon);
                        }
                        else {
...
var ConditionBuilderWithRemoteOptions = amisUi.withRemoteConfig({
    ...
    class_1.prototype.componentDidUpdate = function (prevProps, prevState) {
        ...
        else if (amisCore.isObjectShallowModified(this.props.data, prevProps.data, false, undefined, undefined, 10)) {
