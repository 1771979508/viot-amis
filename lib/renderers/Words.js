        ...
        if (!inTag) {
            ...
            return words.map(function (item, index) {
                return (_J$X_(React.Fragment, { key: index },
                    amisCore.labelToString(item),
                    index === lastIndex_1 ? '' : delimiter ? delimiter : '， '));
            });
        }
        return words.map(function (label, key) { return (
        // 使用tag展示时，默认不使用连接符
        _J$X_(amisUi.Tag, tslib.__assign({ key: key, label: label, className: 'mb-1' }, (typeof inTag === 'object'
            ? tslib.__assign(tslib.__assign({}, inTag), { className: cx(inTag.className) }) : undefined)))); });
    };
