...
import { isObject, filter, createObject, str2function, noop, buildStyle, insertStyle, autobind, themeable, someTree, mapTree, getExprProperties, guid, findTree, isUnfolded, spliceTree, isObjectShallowModified, resolveVariableAndFilter, findTreeIndex, isEffectiveApi, ScopedContext, Renderer, evalExpression } from 'amis-core';
            ...
            link.icon &&
                (Array.isArray(link.icon) ? link.icon : [link.icon]).forEach(function (item, i) {
                    if (React.isValidElement(item)) {
                        ...
                        var icon = (React.createElement(Icon, { key: "icon-".concat(i), cx: cx, icon: item['icon'] || item, className: isCollapsedNode ? '' : isAfter ? 'ml-2' : 'mr-2' }));
                        if (isAfter) {
                            afterIcon.push(icon);
                        }
                        else {
        ...
        if (style) {
            try {
                ...
                if (!document.getElementById(classNameId)) {
                    ...
                    insertStyle({
                        style: ".".concat(classNameId, " ").concat(styleText),
                        classId: classNameId
                    });
                }
            }
...
var ConditionBuilderWithRemoteOptions = withRemoteConfig({
    ...
    class_1.prototype.componentDidUpdate = function (prevProps, prevState) {
        ...
        else if (isObjectShallowModified(this.props.data, prevProps.data, false, undefined, undefined, 10)) {
...
/** @class */ ((function (_super) {
    ...
    NavigationRenderer.prototype.doAction = function (action, data, throwErrors, args) {
        var _a;
